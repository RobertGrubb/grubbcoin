...
const char * const BITCOIN_CONF_FILENAME = "grubbcoin.conf";
...
#else
    const char* pszModule = "grubbcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GrubbCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/GrubbCoin";
#else
    // Unix
    return pathRet / ".grubbcoin";
#endif
#endif
}
